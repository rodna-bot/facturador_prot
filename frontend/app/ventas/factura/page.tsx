...
export default function FacturaPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <div className="bg-gradient-to-b from-blue-50 to-transparent border-b border-blue-200 p-2">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-lg font-bold">Crear Factura 🧾 - Sistema de Inventario</h1>
        </div>
        <p className="text-xs text-muted-foreground ml-9">
          Comprobante de pago para operaciones comerciales de acuerdo a SUNAT
        </p>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-3 py-2 space-y-2">
          ...
          <CardContent className="space-y-2 pt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <div className="space-y-1">
                <Label className="text-xs">RUC (Requerido)</Label>
                <div className="flex gap-1">
                  ...
                  />
                  <Button onClick={buscarRUC} disabled={ruc.length < 11} size="sm" className="h-7 text-xs">
                    Buscar
                  </Button>
                </div>
              </div>
            ...
            {clienteData && (
              <div className="bg-blue-50 p-2 rounded-lg border border-blue-200 space-y-1">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
                  <div>
                    <p className="text-xs text-muted-foreground">Razón Social</p>
                    <p className="font-semibold text-xs">{clienteData.razonSocial}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Dirección</p>
                    <p className="font-semibold text-xs">{clienteData.direccion}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Distrito</p>
                    <p className="font-semibold text-xs">{clienteData.distrito}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Departamento</p>
                    <p className="font-semibold text-xs">{clienteData.departamento}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Datos de la Factura</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-2 pt-0">
            <div className="space-y-1">
              <Label className="text-xs">Serie</Label>
              <Input placeholder="F001" value="F001" disabled className="h-7 text-xs" />
            </div>
            <div className="space-y-1">
            ...
            <div className="flex items-start gap-2 p-2 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <Label htmlFor="guia-remision" className="flex-1 cursor-pointer">
                <div className="space-y-0">
                  ...
                  <p className="text-xs text-muted-foreground">Se enviará documento de transporte junto con la factura</p>
                </div>
              </Label>
            </div>
          </CardContent>
        ...
        <div className="flex gap-2 justify-end sticky bottom-24">
          ...
          <Button disabled={items.length === 0 || !clienteData} className="gap-2">
            <Send className="h-4 w-4" />
            Emitir Factura {generarGuiaRemision && "y Guía"}
          </Button>
        </div>
      </div>
    </div>
